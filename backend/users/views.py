...
class TelegramAuthView(APIView):
    ...
    def post(self, request):
        ...
        if not check_hash:
            return Response({'detail': 'Missing hash.'}, status=400)

        bot_token = getattr(django_settings, 'TELEGRAM_BOT_TOKEN', '')
        if not bot_token:
            return Response({'detail': 'Telegram auth not configured.'}, status=500)

        data_check_string = '\n'.join(f'{k}={v}' for k, v in sorted(data.items()))
        secret_key        = hashlib.sha256(bot_token.encode()).digest()
        computed          = hmac_mod.new(secret_key, data_check_string.encode(), hashlib.sha256).hexdigest()

        if not hmac_mod.compare_digest(computed, check_hash):
            return Response({'detail': 'Invalid Telegram authentication.'}, status=400)

        if time.time() - int(data.get('auth_date', 0)) > 86400:
            return Response({'detail': 'Auth data expired. Please try again.'}, status=400)
        ...
        if not user:
            if not invite_token:
                return Response({'detail': 'No account linked to this Telegram. Use an invite link to register.'}, status=404)

            from academies.models import InviteToken
            try:
                invite = InviteToken.objects.get(token=invite_token)
            except InviteToken.DoesNotExist:
                return Response({'detail': 'Invalid or expired invite.'}, status=400)
            if not invite.is_valid:
                return Response({'detail': 'This invite has expired or reached its use limit.'}, status=400)

            base = tg_username or f'tg_{telegram_id}'
            username, n = base, 1
            ...
            user = User.objects.create_user(
                ...
                telegram_id=telegram_id, role=invite.role, academy=invite.academy,
            )
            ...
            if invite.group and invite.role == 'student':
                ...
                membership, created = GroupMembership.objects.get_or_create(group=invite.group, student=user)
                if created:
                    for lesson in invite.group.lessons.all():
                        Attendance.objects.get_or_create(lesson=lesson, student=user, defaults={'present': False})
            ...
            try:
                invite = InviteToken.objects.get(token=invite_token)
                if invite.is_valid and not invite.used_by.filter(pk=user.pk).exists():
