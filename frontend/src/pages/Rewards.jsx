...
import { Gift, Plus, Loader2, Star } from 'lucide-react'
import { getRewards, createReward } from '../api/rewards'
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 18 }}>
          {visible.map((r, i) => <RewardCard key={r.id} reward={r} index={i} />)}
        </div>
      )}

      <CreateRewardModal open={showCreate} onClose={() => setShowCreate(false)}
        onCreated={r => { setRewards(rs => [r, ...rs]); setShowCreate(false); show(t('rewards.toast_created'), 'success') }} />
    </div>
  )
}
...
function CreateRewardModal({ open, onClose, onCreated }) {
  const { show } = useToast(); const { t } = useTranslation()
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({ name: '', description: '', icon: '', price: '', stock: 0, category: 'other', status: 'coming_soon' })
  const [error, setError] = useState('')

  useEffect(() => {
    if (open) { setForm({ name: '', description: '', icon: '', price: '', stock: 0, category: 'other', status: 'coming_soon' }); setError('') }
  }, [open])
  ...
  const submit = async e => {
    ...
    setLoading(true)
    try {
      const { data } = await createReward({ ...form, price: Number(form.price), stock: Number(form.stock) || 0 })
      onCreated(data)
    } catch {
      show(t('rewards.toast_create_fail'), 'error')
    } finally { setLoading(false) }
  }

  return (
    <Modal open={open} onClose={onClose} title={t('rewards.create_modal_title')}>
      <form onSubmit={submit}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 24 }}>
          ...
          <motion.button type="submit" disabled={loading} whileHover={{ translateY: -1 }} whileTap={{ scale: 0.97 }} style={{ ...primaryBtn, opacity: loading ? 0.7 : 1 }}>
            ...
            {loading ? t('rewards.creating') : t('rewards.create_btn')}
          </motion.button>
        </div>
      </form>
    </Modal>
